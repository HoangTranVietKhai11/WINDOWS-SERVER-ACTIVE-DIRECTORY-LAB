...
## Mục tiêu
Tăng cường mức độ an toàn thông tin cho hệ thống máy trạm bằng cách chặn truy cập Command Prompt, khóa thiết bị lưu trữ USB di động và kiểm soát ứng dụng được phép chạy thông qua AppLocker.
...
- Mở **gpmc.msc**, tạo hoặc chỉnh sửa GPO áp dụng cho khối Nhân sự / Kinh doanh (ví dụ `GPO_Security_Hardening`).
- Đường dẫn:
...
### 4.3 Kiểm soát phần mềm bằng AppLocker
- Đường dẫn:
...
- **Thêm Custom Rule (Cấm phần mềm ngoài danh mục)**:
  ...
  - *User or group*: `Everyone` hoặc nhóm chỉ định (ví dụ `G_NhanSu`).
...
5. **Thử nghiệm 3 (Test AppLocker)**: Chạy file `.exe` không nằm trong danh mục cho phép.
